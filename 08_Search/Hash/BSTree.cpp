...
TreeNode<TLinklist>::TreeNode(TLinklist key, TLinklist *data)
{
	this->key = key;
	this->data = data;
	this->bRight = NULL;
...
TLinklist BSTree<TLinklist>::RecursiveInsert(TreeNode<TLinklist> *RootNode, TreeNode<TLinklist> *ChiNode)
{
	...
	{
		if(BSTreeCompare(RootNode->key, ChiNode->key) > 0)
		{
		...
		else if(BSTreeCompare(RootNode->key, ChiNode->key) < 0)
		{
...
template <class TLinklist>
TLinklist BSTree<TLinklist>::InsertBSTree(TLinklist key, TLinklist *data) //在树中的某个位置添加元素
{
	TLinklist ret = FALSE;
	TreeNode<TLinklist> *tNode = new TreeNode<TLinklist>(key, data);
...
TreeNode<TLinklist>* BSTree<TLinklist>::RecursiveGet(TreeNode<TLinklist>* ChiNode, TLinklist Des)
{
	...
	if(ChiNode != NULL)
	{
		if(BSTreeCompare(ChiNode->key, Des) == 0)
		{
			ret = ChiNode;
		}
		else if(BSTreeCompare(ChiNode->key, Des) > 0)
		{
			if(ChiNode->bLeft != NULL)
			{
				ret = RecursiveGet(ChiNode->bLeft, Des);
			}
		}
		else if(BSTreeCompare(ChiNode->key, Des) < 0)
		{
			...
			{
				ret = RecursiveGet(ChiNode->bRight, Des);
			}
		}
	}
	return ret;
